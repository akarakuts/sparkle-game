...
const GRID_ROWS: int = 4  # 4x4 = 16 карт (8 пар)
...
func _update_attempt_counter() -> void:
	MiniGameArt.render_digit_row(_attempt_digits, str(_attempts), DIGIT_TEXTURES)
...
func _build_grid() -> void:
	...
	var grid_width: float = GRID_COLS * CARD_SIZE.x + (GRID_COLS - 1) * CARD_GAP
	var grid_height: float = GRID_ROWS * CARD_SIZE.y + (GRID_ROWS - 1) * CARD_GAP
